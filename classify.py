...
def deepspeech_recog(path):
    filename = path.split('/')[-1]
    input = path
    # restore_path = './DeepSpeech/deepspeech-0.4.1-checkpoint/model.v0.4.1'
    restore_path = '/data/guofeng/Speech_Recognition/transferability_check/api/DeepSpeech/deepspeech-0.4.1-checkpoint/model.v0.4.1'
    while len(sys.argv) > 1:
        sys.argv.pop()
    with tf.Session() as sess:
        ...
        r = sess.run(decoded, {new_input: [audio],
                               lengths: [length]})
        result = "".join([toks[x] for x in r[0].values])
...
if __name__ == '__main__':
    # main()
    path = '//sample-000000.wav'
    r, t = deepspeech_recog(path)
    print(r, t)
